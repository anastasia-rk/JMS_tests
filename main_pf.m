...
sample_x0 = @(x) mvnrnd(x, 0.1*eye(4));               % initial distribution
...
%       psys.index   - particles weights at time t-1, size (N_p x t)
%   functions:
...
for it=1:t-1
    cols{it} = (c_max*psys.weights(:,it)); %linspace(1,10,psys.np);
    scatter([it*ones(1,psys.np)],psys.x_1t{ix}(it,:),cz,cols{it},'filled'); hold on;
    scatter(it,psys.x_1t{ix}(it,:)*psys.weights(:,it),50,'r','filled');hold on;
end
hcb = colorbar;
title(hcb,'$w_i$','fontweight','normal','interpreter','latex','fontsize',16)
% Label.Interpreter = 'latex';
legend('True state','Particles','Weighted estimates')
end
%%
figure;
...
for im=1:psys.nm
    ...
    colormap(my_map);
    for it=1:t-1
        scatter([it*ones(1,psys.np)],psys.mu_tt{im}(it,:),25,cols{it},'filled'); hold on;
        scatter(it,psys.mu_tt{im}(it,:)*psys.weights(:,it),50,'r','filled');hold on;
    end
    plot(mu_weighted{k}(im,:),'k','Linewidth',2); hold on;
    hcb = colorbar;
    title(hcb,'$w_i$','fontweight','normal','interpreter','latex','fontsize',16)
    title(['$\mu(M^',num2str(im),')$']);
    legend('Particles','Weighter estimates')
end
